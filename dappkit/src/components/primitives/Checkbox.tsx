...
export default function Checkbox({ look, size, state, className, ...props }: CheckboxProps) {
  ...
  return (
    <Root
      ...
      className={mergeClass(checkboxStyles({ look: look ?? "base", size: size ?? "md" }))}
      defaultChecked
      id="c1"
    >
      x
      <Indicator className="text-violet11">x</Indicator>
    </Root>
  );
}
