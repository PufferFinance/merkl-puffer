...
export default function OpportunityTableRow({ hideTags, opportunity, className, ...props }: OpportunityTableRowProps) {
  return (
    <OpportunityRow
      ...
      size="lg"
      content="sm"
      className={mergeClass("!p-lg*2", className)}
      {...props}
      aprColumn={
        <Group>
          ...
          <Group className="text-nowrap whitespace-nowrap text-ellipsis min-w-0 flex-nowrap overflow-hidden max-w-full">
            <Icons>
              {opportunity?.contextTag
                ...
                .map(({ value }) => (
                  <Icon size={props?.size} src={value?.logoURI} />
                ))}
            </Icons>
            <Title h={3} className="text-nowrap whitespace-nowrap text-ellipsis min-w-0">{opportunity.name}</Title>
          </Group>
          <Group>
            {opportunity?.contextTag
